...
const btnSinal = document.getElementById("btnSinal");
const btnParenteses = document.getElementById("btnParenteses");
...
numeros.forEach(botao => {
    botao.addEventListener("click", () => {
        if (display.value === "Erro")
            return; //Retorne se display for erro e nao deixa fazer nada
...
btnOp.forEach(botao => {
    botao.addEventListener("click", () => {
        ...
        if (display.value === "" && (operador === "*" || operador === "/"))
            return; //Nao permite os clicar em * ou / se o display nao tiver nada
        if (display.value.slice(0, -1) === "" && /[+\-]/.test(ultimoChar) && (operador === "*" || operador === "/"))
            return; // nao permite clicar em * ou / se o caracter anterior for + ou - e eles forem os primeiros caracteres do display
        if (/[(]/.test(ultimoChar) && (operador === "*" || operador === "/"))
            return; //Se o ultimo caracter for ( e o botao clicado for * ou / retorne
...
btnPorcentagem.addEventListener("click", () => {
    ...
    if (display.value === "" || display.value === "Erro")
        return; //se for o primeiro ou se o display estiver com erro retorne
...
btnPonto.addEventListener("click", () => {
    ...
    if (numeroJaTemPonto)
        return; //se ja tem um ponto retorne
    if (/[+\-*/(]/.test(ultimochar)) {
...
btnParenteses.addEventListener("click", () => {
    ...
    if (display.value === "Erro" || ultimochar === ".")
        return;
    const expressao = display.value;
