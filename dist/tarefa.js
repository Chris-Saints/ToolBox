"use strict";
const receberItem = document.getElementById("receber-item"); // Recebe o input do Html
const botao = document.getElementById("botao"); //Recebe o botao do Html
const listaTarefaConcluida = document.getElementById("tarefas-concluidas");
const listaDeTarefa = document.getElementById("adicionar-tarefa"); //recebe o elemento ul do html onde ficara a lista de tarefas
function salvarTarefas() {
    const tarefas = []; //criou o array onde vao ficar
...
function recuperarTarefas() {
    ...
    if (!arquivos)
        return; //se nao houver nenhuma informacao no localstorage nada acontece
    const tarefas = JSON.parse(arquivos); //Cria uma const array onde recebe todas as informacoes salvas no arquvios e transforma de string para seus valores reais anteriores
...
function criarTarefa(tarefa, completed = false) {
    ...
    editar.addEventListener("click", () => {
        const previousInput = document.querySelector("li input[type='text']"); //Procura se existe alguma tarefa com input de texto ativo
        const previousEditar = document.querySelector("li button.editing"); //Procura o botao que esta com a classe editar, ou seja aquele que esta com a palavra Salvar
        ...
        if (editar.innerHTML === '<img src="../Assets/edit_icon.svg" alt="Editar">') {
            ...
            const input = li.querySelector("input[type= 'text']"); //Pega o campo que esta sendo input dentro de um li
